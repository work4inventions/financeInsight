...
const IncomeRound = () => {
  ...
  const tagData = income.reduce((acc, inc) => {
    const tag = inc.tag || "Others";
    if (!acc[tag]) {
      acc[tag] = { tag, amount: 0 };
    }
    acc[tag].amount += inc.amount;
    return acc;
    ...
    <div>
      <Card className="flex flex-col bg-black">
        <CardHeader className="items-center pb-0">
          ...
          <CardDescription>Income by Tag</CardDescription>
        </CardHeader>
